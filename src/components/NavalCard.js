...
const NavalCard = ({ naval }) => {
  const [showModal, setShowModal] = useState(false)
  const [flagError, setFlagError] = useState(false)
  const [imageLoaded, setImageLoaded] = useState(false)

  // Check if 3D model is available
  const has3DModel = naval.sketchfab_embed_url && naval.sketchfab_embed_url !== "NOT FOUND"
    ...
    <div style={cardContainerStyle}>
      ...
      <div style={cardInnerStyle}>
        <div style={imageContainerStyle}>
          <img
            src={naval.image_url || "/placeholder.svg"}
            alt={naval.name}
            style={{
              ...imageStyle,
          ...
          <div style={countryBadgeStyle}>
            {!flagError && (
              <img
                src={`/data/flags/${naval.country.toLowerCase()}.jpg`}
                alt={naval.country}
                style={flagStyle}
                onError={(e) => {
                  setFlagError(true)
            ...
            <span style={countryTextStyle}>{naval.country}</span>
          </div>

          {/* Service branch indicator */}
          <div style={serviceBadgeStyle}>
            <div style={statusLightStyle}></div>
            <span style={serviceTextStyle}>{naval.service}</span>
          </div>
        ...
        <div style={contentStyle}>
          {/* naval designation header */}
          <div style={headerSectionStyle}>
            <h2 style={navalNameStyle}>{naval.name}</h2>
            <div style={roleBadgeStyle}>
              <span style={roleTextStyle}>{naval.role}</span>
            </div>
          </div>
          ...
          <div style={modelSectionStyle}>
            ...
            <span style={modelValueStyle}>{naval.model}</span>
          </div>
          ...
          <div style={flightDataStyle}>
            <div style={dataRowStyle}>
              ...
              <span style={dataValueStyle}>{naval.units}</span>
            </div>
            <div style={dataRowStyle}>
              <span style={dataLabelStyle}>STATUS:</span>
              <span style={assessmentStyle}>{naval.assessment}</span>
            </div>
          </div>
          ...
          <div style={missionBriefStyle}>
            ...
            <p style={descriptionStyle}>{naval.description}</p>
          </div>
          ...
          <div style={modalContainerStyle}>
            ...
            <div style={modalHeaderStyle}>
              <div style={modalTitleSectionStyle}>
                ...
                <h3 style={modalTitleStyle}>NAVAL INSPECTION: {naval.name.toUpperCase()}</h3>
              </div>
              <button
            ...
            <div style={viewerContainerStyle}>
              <div style={viewerFrameStyle}>
                <iframe
                  title={`${naval.name} 3D Model`}
                  frameBorder="0"
                  allowFullScreen
                  mozallowfullscreen="true"
                  webkitallowfullscreen="true"
                  allow="autoplay; fullscreen; xr-spatial-tracking"
                  src={naval.sketchfab_embed_url}
                  style={iframeStyle}
                ></iframe>
              </div>
            </div>
